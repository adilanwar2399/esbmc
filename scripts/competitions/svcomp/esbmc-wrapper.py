...
def get_command_line(strat, prop, arch, benchmark, concurrency, dargs):
  ...
  if prop == Property.overflow:
    ...
    if concurrency:
      ...
      command_line += "--no-pointer-check --no-bounds-check --interval-analysis --interval-analysis-assume-asserts --interval-analysis-eval-assumptions --interval-analysis-simplify --error-label ERROR --goto-unwind --unlimited-goto-unwind "
  else:
